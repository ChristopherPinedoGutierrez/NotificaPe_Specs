...
* **Descripci�n:** Integraci�n de Firebase Cloud Messaging (FCM) SDK y sincronizaci�n de FCM Token en el login para la arquitectura Push-to-Pull.
* **Detalles T�cnicos:**
...
* **Criterios de Aceptaci�n (AC) Validados:**
  ...
  - [x] AC 2: Forzar actualizaci�n del FcmToken en la base de datos Supabase durante el login de Google y Correo.
---
---
### [2026-09-15 11:55] | App/Componente: Viewer | Autor: AGENT_ROLE

* **Descripci�n:** Solucionado error cr�tico en encolamiento de notificaciones (Deadlock en TtsManager).
* **Detalles T�cnicos:**
  ...
  - **Base de Datos:** Ninguno. Se reestructur� la inicializaci�n del TTS para evitar que un fallo en el motor deje bloqueado el coroutine channel de NotificationQueueManager, lo cual evitaba que se reprodujeran las alertas de voz y las vibraciones subsecuentes.
* **Criterios de Aceptaci�n (AC) Validados:**
  - [x] AC 1: Las notificaciones encoladas se rechazan o prosiguen limpiamente si el TTS no est� disponible, en lugar de bloquear el worker.
---
---
### [2026-09-15 12:57] | App/Componente: Viewer | Autor: AGENT_ROLE

* **Descripci�n:** Correcci�n cr�tica en la propagaci�n de notificaciones FCM estando la app cerrada y correcci�n de logs.
* **Detalles T�cnicos:**
...
ull en el ID del contratante.
  ...
  - [x] AC 3: Los logs de BackgroundSyncManager incluyen [FCM-FLOW: CATCH-UP] para su f�cil rastreo en el Logcat.
---
---
### [2026-09-16 22:09] | App/Componente: Viewer | Autor: AGENT_ROLE

* **Descripción:** Estabilización de Agrupamiento Nativo de FCM y Límite de Cola de Voz.
* **Detalles Técnicos:**
  ...
  - **Base de Datos:** Ninguno. Se revirtió el experimento manual de InboxStyle hacia el agrupamiento nativo de Android corrigiendo el cruce de NotificationChannels (Silence vs HeadsUp). Se limitó la cola FIFO de alertas a un máximo de 10 posiciones (BufferOverflow.DROP_OLDEST) para prevenir desbordamientos de TTS ante tormentas FCM. Se corrigió el prefijo del título de la notificación persistente.
* **Criterios de Aceptación (AC) Validados:**
  - [x] AC 1: Android agrupa nativamente las notificaciones en un Dropdown único.
  - [x] AC 2: La cola de TTS no reproduce más de 10 pagos si llegan en cascada masiva.
  - [x] AC 3: Título persistente fijo a "NotificaPe Viewer: ...".
---

---
### [2026-09-18 13:29] | App/Componente: Viewer | Autor: Orquestador SDD

* **Descripci�n:** Implementaci�n de payload espec�fico FCM para evitar duplicidad de alertas push y precisi�n en los cambios de permisos.
* **Detalles T�cnicos:**
...
* **Criterios de Aceptaci�n (AC) Validados:**
  - [x] AC 1: Alertas persistentes y emergentes actualizan el idAutorizacion evadiendo acumulaci�n excesiva (badge count drop).
---
...
* **Descripci�n:** Refinamiento de notificaciones: Eliminada actualizaci�n redundante de persistente y mejora de textos.
* **Detalles T�cnicos:**
  - **Archivos Modificados:** [FCMReceiverService.kt](file:///C:/Trabajo/Proyectos/NotificaPe/viewer/app/src/main/java/com/notificape/viewer/service/FCMReceiverService.kt)
  - **Manejo UI:** Se removi� updateStateNotification del FCM; ahora delega 100% a MainViewModel.
* **Criterios de Aceptaci�n (AC) Validados:**
  - [x] AC 1: Textos espec�ficos refieren a dispositivo y empresa, evadiendo solapamiento con notificaci�n persistente.
---

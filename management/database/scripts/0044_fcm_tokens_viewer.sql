...
BEGIN
    ...
    IF TG_TABLE_NAME = 'AutorizacionesXUsuario' THEN
        IF TG_OP = 'UPDATE' THEN
            -- Solo notificar si cambió el estado de aprobación o la conexión
            IF OLD."IdEstadoAuth" IS DISTINCT FROM NEW."IdEstadoAuth" OR OLD."IsConnected" IS DISTINCT FROM NEW."IsConnected" THEN
                SELECT "FcmToken" INTO target_token FROM public."Usuarios" WHERE "IdUsuario" = NEW."IdUsuario";
                IF target_token IS NOT NULL THEN
                    payload := jsonb_build_object('action', 'SYNC_AUTH', 'target', target_token);
    ...
    -- Bucle de envío cruzado: Enviar Push a todos los Cajeros Viewer autorizados a la caja afectada
    IF dispositivo_id IS NOT NULL THEN
        FOR usuario_record IN
            ...
            WHERE a."IdDispositivo" = dispositivo_id 
              ...
              AND a."IsConnected" = true
              AND u."FcmToken" IS NOT NULL
        LOOP
            IF TG_TABLE_NAME = 'NotificacionesXDispositivo' OR TG_TABLE_NAME = 'NotificacionesAUsuarios' THEN
                payload := jsonb_build_object('action', 'SYNC_PAYMENTS', 'target', usuario_record."FcmToken");
            ELSIF TG_TABLE_NAME = 'BilleterasXDispositivo' THEN
                payload := jsonb_build_object('action', 'SYNC_WALLETS', 'target', usuario_record."FcmToken");
            END IF;
            PERFORM net.http_post(url := edge_function_url, headers := auth_header, body := payload);
        END LOOP;
    END IF;
